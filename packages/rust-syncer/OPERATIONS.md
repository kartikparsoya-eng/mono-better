...
| `rate(zero.sync.failed-client-groups) > ~0.1/s` sustained | CGs are being torn down (flush failures, load errors, panics). Correlate with logs `"CG <id>: config_and_hydrate failed: …"` / `"advance_and_sync failed: …"` / `"unable to load CVR: …"`; the clients receive the same underlying error text as an `Internal` error (TS `wrapWithProtocolError`). |
...
| **Rehome semantics** | The client receives a retryable `Rehome` error and reconnects; the LB places it on another instance where it re-hydrates from its CVR. Used for: drain, `MAX_CLIENT_GROUPS` overflow, slow-client shed (INVENTIONS I-4). It is *reconnect elsewhere*, not *reset* (contrast `ClientNotFound`, §6). A generic `fail_group` (flush/load/advance failure) does NOT Rehome since 2026-09-03: it sends `{kind: Internal, message: <underlying error>}` like TS `#cleanup(err)` → `client.fail(err)`, and zero-client backs off before reconnecting. |
...
| **CVR pool saturation** <!-- sync_engine.rs:370-410, metrics.rs pool gauges --> | Pool acquire timeout is 10 s. A failed flush is retried **once** with 100–300 ms jitter (flush is one PG transaction — retry is safe); a second failure → `fail_group` → all the group's clients get an `Internal` error carrying the flush error text (TS parity, 2026-09-03; was `Rehome`). | Alert on idle==0 + flush errors (see §4). Increase `cvr.maxConns` (TS flag; becomes `CVR_MAX_CONNS`) or scale out syncers. |
