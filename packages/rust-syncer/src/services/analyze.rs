...
//!   needs a fresh `Database` handle and is deferred.
//! - `createSQLiteCostModel` planner wiring + `joinPlans` (see `run_ast`).

use crate::protocol::analyze_query_result::AnalyzeQueryResult;

use super::run_ast::run_ast;
...
pub fn analyze_query(
    ...
    run_ast(&mut pipelines, ast_json, synced_rows, vended_rows)
}
