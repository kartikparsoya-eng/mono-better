...
mod tests {
    ...
    /// Layer-2 body-differential: `is_query_covered_by` must return the same bool
    /// as the REAL TS `isQueryCoveredBy` for every AST pair in
    /// `query-covering-fixture.json` (generated by
    /// `generate-query-covering-fixture.mjs`) — pinning filter-subset, IN/range
    /// implication, OR coverage, limit/paging, recursive related, and NOT-EXISTS
    /// reversal to TS.
    #[test]
    fn query_covering_parity_against_ts() {
        let path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/agentic/parity/query-covering-fixture.json"
        );
        let bytes = std::fs::read(path)
            .unwrap_or_else(|e| panic!("failed to read query-covering fixture {path}: {e}"));
        let fixture: Value =
            serde_json::from_slice(&bytes).expect("query-covering fixture is not valid JSON");
        let cases = fixture["cases"].as_array().expect("fixture.cases missing");
        assert!(!cases.is_empty());
        for case in cases {
            let desc = case["desc"].as_str().unwrap_or("");
            let got = is_query_covered_by(&case["covered"], &case["covering"]);
            let want = case["covered_by"].as_bool().unwrap();
            assert_eq!(got, want, "covered_by mismatch for case: {desc}");
        }
    }
