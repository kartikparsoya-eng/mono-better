...
//! on every object of the [`strict`] stamp, enums for every literal union, and `optional_no_null` on
...
/// The schema in both valita parse modes. valita picks the mode per call
/// (`validator.parse(json, {mode})`) while serde fixes `deny_unknown_fields`
/// per type, so one schema body is stamped out twice — Rust-only machinery
/// for a TS parameter, not a second schema. [`strict`] is valita's default
/// (`v.parse(value, upstreamSchema)`, the client boundary) and rejects
/// unknown keys; [`passthrough`] is `fetchFromAPIServer`'s
/// `{mode: 'passthrough'}` (custom/fetch.ts:260) for the API server's
/// response — unknown keys are kept, every known field is still checked.
...
/// `astSchema` as parsed at the upstream boundary (valita `strict`).
pub mod strict {
    use super::*;
    ast_schema!(#[serde(deny_unknown_fields)]);
}

/// `astSchema` as parsed from the API server (valita `passthrough`).
pub mod passthrough {
    use super::*;
    ast_schema!();
}
...
mod tests {
    ...
    /// valita `passthrough` (custom/fetch.ts:260): unknown keys at any level
    /// are kept, while a wrong type, a literal outside its union, and `null`
    /// for an `.optional()` field still fail. The strict stamp rejects the
    /// first of these, so the two stamps are observably different.
    #[test]
    fn passthrough_keeps_unknown_keys_and_still_checks_known_fields() {
        let with_extras = serde_json::json!({
            "table": "issue",
            "extra": 1,
            "where": {"type": "simple", "op": "=", "left": {"type": "column", "name": "id", "bogus": true},
                      "right": {"type": "literal", "value": "a"}, "unknown": null},
            "orderBy": [["id", "asc"]],
        });
        assert!(passthrough::Ast::deserialize(&with_extras).is_ok());
        assert!(strict::Ast::deserialize(&with_extras).is_err());

        let wrong_type = serde_json::json!({"table": 5});
        assert!(passthrough::Ast::deserialize(&wrong_type).is_err());
        let bad_literal = serde_json::json!({"table": "issue", "orderBy": [["id", "up"]]});
        assert!(passthrough::Ast::deserialize(&bad_literal).is_err());
        let null_optional = serde_json::json!({"table": "issue", "limit": null});
        assert!(passthrough::Ast::deserialize(&null_optional).is_err());
    }
}
