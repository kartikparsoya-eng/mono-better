    ...
    let outcome = loop {
        ...
        match send_result {
            Err(e) => {
                ...
                crate::metrics::record_api_attempt("fetch_error", will_retry, attempt_ms);
                if will_retry {
            ...
            Ok(resp) => {
                ...
                if !status.is_success() {
                    ...
                    crate::metrics::record_api_attempt("http_error", will_retry, attempt_ms);
                    if will_retry {
                        tokio::time::sleep(Duration::from_millis(backoff_delay_ms(attempt))).await;
                        attempt += 1;
                        continue;
                ...
                match resp.json::<Value>().await {
                    Ok(v) => {
                        crate::metrics::record_api_attempt("success", false, attempt_ms);
                        break Ok(v);
                    }
                    Err(e) => {
                        crate::metrics::record_api_attempt("parse_error", false, attempt_ms);
                        break Err((
                            "parse_error",
                            transform_failed(
                                "Internal",
