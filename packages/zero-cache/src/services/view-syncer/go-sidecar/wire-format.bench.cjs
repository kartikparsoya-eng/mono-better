...
function rowVals(i) {
  return [
    'conv_' + String(i).padStart(12, '0'),
    'chan_' + String(i % 5000).padStart(8, '0'),
    'ws_000007',
    'user_' + String(i % 900).padStart(6, '0'),
    'Re: incremental view maintenance throughput thread',
    1779813865070 + i * 1000,
    1779813999070 + i * 1000,
    1779814100070 + i * 1000,
    i % 40,
    i % 12,
    100 + (i % 4000),
    i % 7 === 0,
...
function makePositional(n) {
  ...
  return {
    q: 'q_channelConversationsPaginatedV3_abc123',
    t: 'conversations',
    c: cols,
    r,
  };
...
function timeit(fn, iters) {
  ...
  return Number(process.hrtime.bigint() - t) / iters / 1000;
}
...
for (const n of [100, 1000, 10000]) {
  const base = makeBaseline(n),
    pos = makePositional(n);
  const baseEnc = packr.pack(base),
    posEnc = packr.pack(pos);
  for (let i = 0; i < 20; i++) {
    unpackr.unpack(baseEnc);
    unpackr.unpack(posEnc);
  } // warm
  ...
  const posDecRebuild = timeit(() => {
    const g = unpackr.unpack(posEnc);
    const C = g.c;
    const out = new Array(g.r.length);
    for (let i = 0; i < g.r.length; i++) {
      const arr = g.r[i];
      const row = {};
      for (let j = 0; j < C.length; j++) row[C[j]] = arr[1 + j];
      out[i] = {
        type: arr[0],
        queryID: g.q,
        table: g.t,
        rowKey: {conversationId: arr[1]},
        row,
      };
    }
    return out;
  ...
  console.log(
    [
      ...
      baseDec.toFixed(1),
      posDec.toFixed(1),
      posDecRebuild.toFixed(1),
      baseEnc.length,
      posEnc.length,
      `(dec ${(baseDec / posDec).toFixed(2)}x, dec+rebuild ${(baseDec / posDecRebuild).toFixed(2)}x, ${(baseEnc.length / posEnc.length).toFixed(2)}x bytes)`,
    ].join('  '),
  );
}
console.log();
