...
export const zeroOptions = {
  ...
  goSidecar: {
    enabled: {
      type: v.boolean().default(true),
      desc: [
        `Offload IVM compute (advance + hydrate) to the in-process Go engine`,
        ...
        `goivm_napi addon + {bold goSidecar.napiLibPath} c-shared library; if the`,
        `engine fails to start, the PipelineDriver falls back to the TS path.`,
      ],
    },
    napiLibPath: {
      type: v.string().default('libgoivm.so'),
      desc: [
        `Path to the libgoivm c-shared library. The default bare name resolves`,
        `via the platform's dynamic-linker search path (LD_LIBRARY_PATH etc.);`,
        `use an absolute path in containers (e.g. /usr/local/lib/libgoivm.so).`,
        `Built in the go-ivm repo with: go build -tags "libsqlite3 napilib"`,
        `-buildmode=c-shared.`,
      ],
    },
    pullWindow: {
      ...
      desc: [
        `Pull-hydration lookahead window W: Go may run at most W row deliveries`,
        `ahead of what the view-syncer consumed; credits top back up to W at the`,
        `W/2 low-water mark. 1 = strict lockstep (literal TS generator semantics,`,
        `one next() per row — the parity-test pin); 64 (default) keeps identical`,
        `semantics with 32x fewer park/wake cycles and boundary calls on the hot`,
        `path.`,
      ],
    },
  },
