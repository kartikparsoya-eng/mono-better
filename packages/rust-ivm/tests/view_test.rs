...
fn test_view_add_plural() {
    ...
    let post2 = make_node(make_row(&[
        ("id", Value::F64(2.0)),
        ("title", Value::Str("second".into())),
    ]));
    let result = apply_change(
        &result,
        &ViewChange::Add {
            node: ViewNode::Lazy(post2),
        },
        &post_schema,
        "posts",
        &format,
    ...
    match result.relationships.get("posts") {
        Some(View::List(entries)) => {
            ...
            assert_eq!(
                entries[0].row.get("title"),
                Some(&Value::Str("first".into()))
            );
            assert_eq!(
                entries[1].row.get("title"),
...
fn test_view_add_duplicate_increments_refcount() {
    ...
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Lazy(make_node(row.clone())),
        },
        &post_schema,
        "posts",
        &format,
...
fn test_view_remove_singular() {
    ...
    let result = apply_change(
        &result,
        &ViewChange::Remove {
            node: ViewNode::Lazy(make_node(row)),
        },
        &profile_schema,
        "profile",
        &format,
...
fn test_view_remove_plural() {
    ...
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Lazy(make_node(row1.clone())),
        },
        &post_schema,
        "posts",
        &format,
    ...
    let result = apply_change(
        &result,
        &ViewChange::Add {
            node: ViewNode::Lazy(make_node(row2.clone())),
        },
        &post_schema,
        "posts",
        &format,
    ...
    let result = apply_change(
        &result,
        &ViewChange::Remove {
            node: ViewNode::Lazy(make_node(row1)),
        },
        &post_schema,
        "posts",
        &format,
...
fn test_view_edit_in_place() {
    ...
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Lazy(make_node(old_row.clone())),
        },
        &post_schema,
        "posts",
        &format,
...
fn test_view_edit_moves_position() {
    ...
    for i in [1, 3, 5] {
        let r = make_row(&[
            ...
            ("title", Value::Str(format!("post{}", i).into())),
        ]);
        result = apply_change(
            &result,
            &ViewChange::Add {
                node: ViewNode::Lazy(make_node(r)),
            },
            &post_schema,
            "posts",
            &format,
...
fn test_view_child_change() {
    ...
    let post_schema = make_schema("post", &["id"]);
    let post_schema =
        post_schema.with_relationship("comments", comment_schema, false, System::Client);
    ...
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Lazy(make_node(post_row)),
        },
        &post_schema,
        "posts",
        &format,
        ...
        Some(View::List(posts)) => {
            ...
            match &posts[0].relationships.get("comments") {
                Some(View::List(comments)) => {
                    ...
                    assert_eq!(
                        comments[0].row.get("text"),
                        Some(&Value::Str("nice".into()))
                    );
                }
...
fn test_view_add_expanded_node() {
    ...
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Expanded(expanded),
        },
        &post_schema,
        "posts",
        &format,
...
fn test_view_add_with_nested_relationship() {
    ...
    let comment_node = make_node(make_row(&[
        ("id", Value::F64(100.0)),
        ("text", Value::Str("first comment".into())),
    ]));
    let mut post_node = make_node(make_row(&[
        ("id", Value::F64(1.0)),
        ("title", Value::Str("hello".into())),
    ]));
        ...
        Some(View::List(posts)) => {
            ...
            match posts[0].relationships.get("comments") {
                Some(View::List(comments)) => {
                    ...
                    assert_eq!(
                        comments[0].row.get("text"),
                        Some(&Value::Str("first comment".into()))
                    );
                }
...
fn test_view_add_with_ids() {
    ...
    let post = make_node(make_row(&[
        ("id", Value::F64(42.0)),
        ("title", Value::Str("tagged".into())),
    ]));
    let result = apply_change(
        &root,
        &ViewChange::Add {
            node: ViewNode::Lazy(post),
        },
        &post_schema,
        "posts",
        &format,
