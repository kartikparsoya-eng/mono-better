...
) -> (Rc<RefCell<MemorySource>>, Rc<RefCell<MemorySource>>, Rc<RefCell<FlippedJoin>>) {
...
fn test_fetch_parent_no_children() {
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(fj.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 0, "Parent with no children = inner join excludes it");
}
...
fn test_fetch_one_parent_one_child() {
    ...
    assert_eq!(children[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("c1"));
}
...
fn test_fetch_one_parent_wrong_child() {
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(fj.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 0, "Child pointing to non-existent parent = no results");
}
...
fn test_fetch_two_parents_each_with_two_children() {
    ...
    assert_eq!(children0[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("c1"));
    assert_eq!(children0[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("c2"));

    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("i2"));
    let children1 = get_rel_children(&nodes[1], "comments");
    assert_eq!(children1.len(), 2);
    assert_eq!(children1[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("c3"));
    assert_eq!(children1[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("c4"));
}
...
fn test_fetch_compound_key() {
    ...
    add_row(&child, &[("id", str_val("c1")), ("pa", str_val("x")), ("pb", str_val("1"))]);
    add_row(&child, &[("id", str_val("c2")), ("pa", str_val("y")), ("pb", str_val("2"))]);
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(fj.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 2, "Both parents should match with compound keys");
...
fn test_fetch_reverse_order() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("i3"));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("i2"));
    assert_eq!(nodes[2].row.get("id").cloned().unwrap_or(Value::Null), str_val("i1"));
}
...
fn test_fetch_many_parents_chunked() {
    ...
    for i in 1..=10 {
        add_row(&child, &[("id", Value::F64(i as f64)), ("pid", Value::F64(i as f64))]);
    }
...
fn test_fetch_hidden_join() {
    let parent = make_source("issues", &["id"], &[("id", ColumnType::String { optional: false })]);
    let child = make_source("comments", &["id"], &[
        ("id", ColumnType::String { optional: false }),
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(fj.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 1, "Hidden join should still return parent nodes");
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("i1"));
}
...
fn test_fetch_start_after() {
    ...
    let start_row: FxHashMap<String, Value> = FxHashMap::from_iter([
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("i3"));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("i4"));
}
...
fn test_fetch_start_at() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("i2"));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("i3"));
}
...
fn test_fetch_start_at_reverse() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), str_val("i2"));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), str_val("i1"));
}
...
fn setup_group_join(
    ...
    for g in 0..num_groups {
        for dup in 0..children_per_group {
            add_row(&child, &[
                ("id", Value::F64((1000 + g * children_per_group + dup) as f64)),
...
fn test_stream_merge_k2_reverse() {
    ...
    let ids: Vec<f64> = nodes
        .iter()
        .map(|n| n.row.get("id").and_then(|v| match v {
            Value::F64(n) => Some(*n),
            _ => None,
        }).unwrap_or(-1.0))
        .collect();
