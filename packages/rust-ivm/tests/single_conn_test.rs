...
fn test_set_db_path_opens_single_connection() {
    ...
    .unwrap();
    conn.execute(
        "INSERT OR REPLACE INTO test_table VALUES ('a', 'Alice')",
        [],
    )
    .unwrap();
    conn.execute("INSERT OR REPLACE INTO test_table VALUES ('b', 'Bob')", [])
        .unwrap();
    drop(conn);
...
fn test_read_only_connection() {
    ...
    let source = make_source(
        "t",
        vec![("id", ColumnType::String { optional: false })],
        &["id"],
    );
