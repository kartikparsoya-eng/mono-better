...
fn source(name: &str, cols: &[(&str, ColumnType)], pk: &[&str]) -> Rc<std::cell::RefCell<MemorySource>> {
...
fn channels_ast() -> Ast {
    Ast {
        ...
        where_clause: Some(Condition::Simple(SimpleCondition {
            ...
            op: "=".to_string(),
            right: ValuePosition::Literal { value: Value::Str(Arc::from("ch1")) },
        })),
        related: vec![],
        limit: None,
        order_by: None,
...
fn double_edit_same_advance_final_state_correct() {
    ...
    let changes1 = engine.advance(&[
        ("channels".to_string(), make_source_change_add(row(vec![
