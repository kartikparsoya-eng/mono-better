...
use rust_ivm::builder::ast::{Ast, Bound, Condition, OrderPart, RelatedSubquery, SimpleCondition, ValuePosition};
...
fn test_take_limit() {
    ...
    for i in 1..=5 {
        source.borrow_mut().add_row(
            [("id".to_string(), Value::F64(i as f64)), ("name".to_string(), Value::Str(format!("user{}", i).into()))]
                .into_iter()
                .collect(),
        );
    }
...
fn test_skip_pagination() {
    ...
    let ast = Ast {
        ...
        limit: None,
        order_by: Some(vec![OrderPart { column: "id".to_string(), direction: "asc".to_string() }]),
        start: Some(Bound { row: start_row, exclusive: true }),
    };

    let results = engine.add_queries(&[QuerySpec { query_id: "q1".to_string(), ast }]);
    // Should skip users 1 and 2, return 3, 4, 5
    assert_eq!(results[0].changes.len(), 3);
    let first_id = results[0].changes[0].row.as_ref().unwrap().get("id").cloned().unwrap_or(Value::Null);
...
fn test_filter_not_equal() {
    ...
    for (id, name) in [(1.0, "Alice"), (2.0, "Bob"), (3.0, "Alice")] {
        source.borrow_mut().add_row(
            [("id".to_string(), Value::F64(id)), ("name".to_string(), Value::Str(name.into()))]
                .into_iter()
                .collect(),
        );
    }
    ...
    let ast = Ast {
        ...
        where_clause: Some(Condition::Simple(SimpleCondition {
            ...
            right: ValuePosition::Literal { value: Value::Str("Alice".into()) },
        })),
        related: vec![],
        limit: None,
        order_by: None,
    ...
    let name = results[0].changes[0].row.as_ref().unwrap().get("name").cloned().unwrap_or(Value::Null);
...
fn test_filter_greater_than() {
    ...
    for i in 1..=5 {
        source.borrow_mut().add_row(
            [("id".to_string(), Value::F64(i as f64)), ("age".to_string(), Value::F64((20 + i) as f64))]
                .into_iter()
                .collect(),
        );
    }
    ...
    let ast = Ast {
        ...
        where_clause: Some(Condition::Simple(SimpleCondition {
            op: ">".to_string(),
            left: ValuePosition::Column { name: "age".to_string() },
            right: ValuePosition::Literal { value: Value::F64(23.0) },
        })),
        related: vec![],
        limit: None,
        order_by: None,
...
fn test_filter_and() {
    let source = make_source("users", &["id"]);
    for (id, name, age) in [(1.0, "Alice", 25.0), (2.0, "Bob", 30.0), (3.0, "Alice", 30.0)] {
        source.borrow_mut().add_row(
            [("id".to_string(), Value::F64(id)), ("name".to_string(), Value::Str(name.into())), ("age".to_string(), Value::F64(age))]
                .into_iter()
                .collect(),
        );
    }
    ...
    let ast = Ast {
        ...
        where_clause: Some(Condition::And(vec![
            ...
            Condition::Simple(SimpleCondition {
                op: "=".to_string(),
                left: ValuePosition::Column { name: "age".to_string() },
                right: ValuePosition::Literal { value: Value::F64(30.0) },
            }),
        ])),
        related: vec![],
        limit: None,
    ...
    let ast = Ast {
        ...
        where_clause: Some(Condition::Or(vec![
            ...
            Condition::Simple(SimpleCondition {
                op: "=".to_string(),
                left: ValuePosition::Column { name: "name".to_string() },
                right: ValuePosition::Literal { value: Value::Str("Carol".into()) },
            }),
        ])),
        related: vec![],
        limit: None,
...
fn test_values_equal_null_semantics() {
    ...
    assert!(!rust_ivm::ivm::data::values_equal(&Value::Null, &Value::Null));
    assert!(rust_ivm::ivm::data::values_equal(&Value::F64(1.0), &Value::F64(1.0)));
    assert!(!rust_ivm::ivm::data::values_equal(&Value::F64(1.0), &Value::F64(2.0)));
    assert!(rust_ivm::ivm::data::values_equal(&Value::Str("hello".into()), &Value::Str("hello".into())));
}

#[test]
fn test_compare_values() {
    use std::cmp::Ordering;
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::F64(1.0), &Value::F64(2.0)), Ordering::Less);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::F64(2.0), &Value::F64(1.0)), Ordering::Greater);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::F64(1.0), &Value::F64(1.0)), Ordering::Equal);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::Str("a".into()), &Value::Str("b".into())), Ordering::Less);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::Null, &Value::F64(1.0)), Ordering::Less);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::F64(1.0), &Value::Null), Ordering::Greater);
    assert_eq!(rust_ivm::ivm::data::compare_values(&Value::Null, &Value::Null), Ordering::Equal);
}
