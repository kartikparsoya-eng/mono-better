...
use rust_ivm::ivm::source::{merge_sorted_streams, MemorySource};
...
fn test_merge_sorted_streams_empty() {
    ...
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(streams, Rc::new(|_: &rust_ivm::ivm::data::Node, _: &rust_ivm::ivm::data::Node| std::cmp::Ordering::Equal))).collect();
...
fn test_merge_sorted_streams_single() {
    ...
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(vec![stream], Rc::new(|a: &rust_ivm::ivm::data::Node, b: &rust_ivm::ivm::data::Node| {
        let av = a.row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0);
        let bv = b.row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0);
...
fn test_merge_sorted_streams_two_streams() {
    ...
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(vec![stream1, stream2], Rc::new(compare))).collect();
...
fn test_merge_sorted_streams_unequal_length() {
    ...
    for i in 0..result.len() {
        let id = result[i].row.get("id").cloned().unwrap_or(Value::Null);
        assert_eq!(id, Value::F64((i + 1) as f64), "Should be merged in order despite unequal stream lengths");
    }
}
...
fn test_merge_sorted_streams_three_streams() {
    ...
    let stream1 = from_vec(vec![Node::new(make_row(&[("id", Value::F64(1.0))])), Node::new(make_row(&[("id", Value::F64(4.0))]))]);
    let stream2 = from_vec(vec![Node::new(make_row(&[("id", Value::F64(2.0))])), Node::new(make_row(&[("id", Value::F64(5.0))]))]);
    let stream3 = from_vec(vec![Node::new(make_row(&[("id", Value::F64(3.0))])), Node::new(make_row(&[("id", Value::F64(6.0))]))]);

    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(vec![stream1, stream2, stream3], Rc::new(compare))).collect();
    assert_eq!(result.len(), 6);
    for i in 0..result.len() {
        let id = result[i].row.get("id").cloned().unwrap_or(Value::Null);
        assert_eq!(id, Value::F64((i + 1) as f64), "Three streams should merge in order");
    }
}

#[test]
fn test_memory_source_fetch_all() {
    let source = make_source("users", &["id"], &[("id", ColumnType::Number { optional: false }), ("name", ColumnType::String { optional: false })]);
    for i in 1..=5 {
        add_row(&source, &[("id", Value::F64(i as f64)), ("name", str_val_owned(format!("user{}", i)))]);
    }

    let input = source.borrow_mut().connect(None, None, None, None);
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&FetchRequest::default())).collect();
...
fn test_memory_source_fetch_with_constraint() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(3.0));
}

#[test]
fn test_memory_source_fetch_with_multi_constraints() {
    let source = make_source("users", &["id"], &[("id", ColumnType::Number { optional: false })]);
    for i in 1..=10 {
...
fn test_memory_source_push_remove() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(2.0));
}

#[test]
fn test_memory_source_push_edit() {
    let source = make_source("users", &["id"], &[("id", ColumnType::Number { optional: false }), ("name", ColumnType::String { optional: false })]);
    add_row(&source, &[("id", Value::F64(1.0)), ("name", str_val("old"))]);
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 1);
    assert_eq!(nodes[0].row.get("name").cloned().unwrap_or(Value::Null), str_val("new"));
}
...
fn test_merge_sorted_streams_equal_keys() {
    ...
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(vec![a, b, c], Rc::new(id_compare))).collect();
    let ids: Vec<f64> = result.iter().map(|n| {
        n.row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0)
    }).collect();
...
fn test_merge_sorted_streams_reverse() {
    ...
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(vec![a, b], Rc::new(reverse))).collect();
    let ids: Vec<f64> = result.iter().map(|n| {
        n.row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0)
    }).collect();
...
fn test_merge_sorted_streams_global_sort_invariant() {
    let data = [3.0, 1.0, 4.0, 1.0, 5.0, 9.0, 2.0, 6.0, 5.0, 3.0, 5.0, 8.0, 9.0, 7.0, 9.0, 3.0];
    ...
    for bucket in buckets.iter_mut() {
        bucket.sort_by(|x, y| id_compare(x, y));
        streams.push(from_vec(bucket.clone()));
    }
    let result: Vec<Node> = rust_ivm::ivm::stream::skip_yields(merge_sorted_streams(streams, Rc::new(id_compare))).collect();
    assert_eq!(result.len(), data.len());
    for i in 1..result.len() {
        let prev = result[i-1].row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0);
        let curr = result[i].row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n), _ => None }).unwrap_or(0.0);
        assert!(curr >= prev, "Global sort invariant violated at index {}: {} < {}", i, curr, prev);
    }
}
...
fn test_memory_source_fetch_reverse() {
    ...
    let input = source.borrow_mut().connect(None, None, None, None);
    let req = FetchRequest { reverse: true, ..Default::default() };
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&req)).collect();
    assert_eq!(nodes.len(), 5);
    for i in 0..5 {
...
fn test_memory_source_fetch_with_start_at() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(3.0));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(4.0));
    assert_eq!(nodes[2].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(5.0));
}
...
fn test_memory_source_fetch_with_start_after() {
    ...
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(4.0));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(5.0));
}
...
fn test_memory_source_fetch_with_filter_predicate() {
    ...
    let predicate: Arc<dyn Fn(&Row) -> bool> = Arc::new(|row| {
        row.get("id").and_then(|v| match v { Value::F64(n) => Some(*n > 2.0), _ => None }).unwrap_or(false)
    });
    let input = source.borrow_mut().connect(None, None, Some(predicate), None);
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 3);
    assert_eq!(nodes[0].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(3.0));
    assert_eq!(nodes[1].row.get("id").cloned().unwrap_or(Value::Null), Value::F64(4.0));
...
fn test_memory_source_fetch_with_multi_constraints_compound_key() {
    ...
    for i in 1..=4 {
        add_row(&source, &[("id", Value::F64(i as f64)), ("a", Value::F64(i as f64)), ("b", str_val_owned(format!("val{}", i)))]);
    }
    let input = source.borrow_mut().connect(None, None, None, None);
    ...
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&req)).collect();
    assert_eq!(nodes.len(), 2, "Compound key multi-constraint should match 2 rows");
}
...
fn test_memory_source_shared_data_visible_after_connect() {
    ...
    add_row(&source, &[("id", Value::F64(2.0))]);
    let nodes: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input.borrow().fetch(&FetchRequest::default())).collect();
    assert_eq!(nodes.len(), 2, "Row added after connect should be visible via shared data");
}
...
fn test_memory_source_multiple_connections_independent() {
    ...
    let nodes1: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input1.borrow().fetch(&FetchRequest::default())).collect();
    let nodes2: Vec<Node> = rust_ivm::ivm::stream::skip_yields(input2.borrow().fetch(&FetchRequest::default())).collect();
