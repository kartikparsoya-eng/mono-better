...
| `totalHydrationTimeMs()`                             | Must match exactly                       | Store the caller's pause-aware `Timer.totalElapsed()` for each successful active query; replace on re-add; delete on remove; clear on reset. Never substitute native actor or consumer-stall wall time.                    |
| advance time budget                                  | Semantically equivalent                  | Charge active computation against the hydration-derived budget and exclude time parked on credit/TSFN delivery, matching TS `TimeSliceTimer` pause semantics. Exact host scheduling time is not compared.                  |
