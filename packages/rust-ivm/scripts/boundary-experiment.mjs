...
db.exec(
  `CREATE TABLE users (id TEXT PRIMARY KEY, name TEXT, email TEXT, age INTEGER, active INTEGER)`,
);
...
async function timeArm(name, fn) {
  ...
  const res = {
    ...
    rows,
    medMs: ms(med),
    perRowUs: med / rows / 1000,
    all: times.map(ms),
  };
  console.log(
    `  [done] ${res.name.padEnd(42)} rows=${res.rows} total=${res.medMs.toFixed(1)}ms per-row=${res.perRowUs.toFixed(3)}µs`,
  );
  return res;
}
...
let nextStreamId = 1;
function streamRows(engine, specs, callback) {
...
arms.noop = await timeArm('stream + no-op onRow (N crossings)', async e => {
  let n = 0;
  await streamRows(e, [{queryId: 'qN', astJson: AST}], () => {
    n++;
  });
  return n;
});
...
arms.real = await timeArm(
  'stream + real onRow (N crossings + parse)',
  async e => {
    let n = 0;
    const sink = [];
    await streamRows(e, [{queryId: 'qR', astJson: AST}], (_err, c) => {
...
async function concSweep(C) {
  ...
  await Promise.all(
    engines.map((e, i) =>
      streamRows(e, [{queryId: `qc${i}`, astJson: AST}], (_err, c) => {
        if (!c || c.changeType < 0) return;
        JSON.parse(c.rowKey);
        if (c.row) JSON.parse(c.row);
        total++;
  ...
  console.log(
    `  [conc C=${String(C).padStart(2)}] streams=${C} rows=${total} wall=${wallMs.toFixed(1)}ms per-row=${perRowUs.toFixed(3)}µs`,
  );
...
async function busyMainArm(burstMs) {
  ...
  const hog = () => {
    if (stop) return;
    if (burstMs > 0) {
      const end = Number(process.hrtime.bigint()) + burstMs * 1e6;
      while (Number(process.hrtime.bigint()) < end) {}
    }
    setImmediate(hog);
  };
  setImmediate(hog);
  let n = 0;
  const t0 = process.hrtime.bigint();
  await streamRows(e, [{queryId: 'qBusy', astJson: AST}], (_err, c) => {
    if (!c || c.changeType < 0) return;
    JSON.parse(c.rowKey);
    if (c.row) JSON.parse(c.row);
    n++;
  ...
  const wallMs = Number(t1 - t0) / 1e6;
  console.log(
    `  [busy burst=${burstMs}ms] rows=${n} wall=${wallMs.toFixed(1)}ms per-row=${(Number(t1 - t0) / n / 1000).toFixed(3)}µs`,
  );
...
// ---- correctness: streaming (current queue depth) must byte-match eager -----
{
  ...
  const streamed = [];
  await streamRows(e2, [{queryId: 'qST', astJson: AST}], (_err, c) => {
    if (c) streamed.push(c);
  });
  try {
    e2.destroy?.();
  } catch {}
...
for (const k of ['eager', 'noop', 'real']) {
  const a = arms[k];
  console.log(
    `  ${a.name.padEnd(42)} rows=${a.rows}  total=${a.medMs.toFixed(1)}ms  per-row=${a.perRowUs.toFixed(3)}µs   [${a.all.map(x => x.toFixed(0)).join(',')}]`,
  );
}
...
console.log(
  `    boundary (noop-eager)/N      = ${boundary.toFixed(3)} µs   <- TSFN round-trip (chunking removes this)`,
);
console.log(
  `    marshal  (real-noop)/N       = ${marshal.toFixed(3)} µs   <- JSON.parse x2 + push`,
);
...
console.log(
  `  eager vs streaming speedup     = ${(arms.real.medMs / arms.eager.medMs).toFixed(1)}x  (what perfect chunking approaches)`,
);
const verdict =
  boundary > marshal
...
console.log(
  `  If per-row balloons with C, the single main JS thread is saturating on delivery`,
);
console.log(
  `  (the production cliff) -> lever A (chunk transport = fewer main-thread callbacks) is primary.`,
);
