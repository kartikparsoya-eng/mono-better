//! MemorySource — port of `zql/src/ivm/memory-source.ts`.
//!
//! The in-memory / test-backed `Source` implementation (plus the SQLite-backed
//! fetch mode via `set_db()`), the overlay machinery, the k-way merge
//! (`merge_sorted_streams`), and the concrete `SourceInput` pusher. The `Source`
//! trait + `SourceChange` types are the `source.ts` twin and live in `source.rs`.
