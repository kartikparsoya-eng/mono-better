...
const addonPath = candidates.find(p => { try { require('fs').readFileSync(p, {flag:'rs'}); return true; } catch { return false; } });
...
function createWhaleDb(dbPath) {
  ...
  db.exec('CREATE TABLE "whale" ("id" TEXT PRIMARY KEY, "title" TEXT NOT NULL, "count" INTEGER NOT NULL, "active" INTEGER NOT NULL, "note" TEXT, "_0_version" TEXT NOT NULL DEFAULT \'0\')');

  const stmt = db.prepare('INSERT OR IGNORE INTO "whale" ("id", "title", "count", "active", "note", "_0_version") VALUES (?, ?, ?, ?, ?, ?)');
  db.exec('BEGIN');
...
async function main() {
  ...
  // --- Streaming path ---
  {
    ...
    await engine.addQueriesStreamingRows(
      ...
      (_err, rc) => { if (rc && rc.changeType >= 0) count++; },
    );
    const rssAfter = process.memoryUsage().rss;
    const deltaMB = (rssAfter - rssBefore) / 1024 / 1024;

    console.log(`Streaming: ${count} rows, RSS delta = ${deltaMB.toFixed(1)} MB (before=${(rssBefore/1024/1024).toFixed(0)} MB, after=${(rssAfter/1024/1024).toFixed(0)} MB)`);
    engine.destroy();
    keeper.close();
  }
  ...
  // --- Eager path ---
  {
    ...
    console.log(`Eager:     ${count} rows, RSS delta = ${deltaMB.toFixed(1)} MB (before=${(rssBefore/1024/1024).toFixed(0)} MB, after=${(rssAfter/1024/1024).toFixed(0)} MB)`);
    engine.destroy();
    keeper.close();
  }
