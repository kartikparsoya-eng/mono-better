...
| `initConnection` validates via `#validateConnection` (TS:942) | `handle_desired_queries(ConfigPassOrigin::InitConnection, ..)` probes before the config pass; the connect-time `client-fallback` record is GONE (TS registers with `revalidateAt: undefined`, connection-context-manager.ts:267) | test proven-on-revert (`init_connection_validates_before_serving_any_data`) | complete |
...
| `#validateConnection` threads the API server's authoritative userID | `transform_query::validate` → `ConnectionValidation`; `transform` → `HashedTransformResponse{cached, validation}`; both `validate_connection` sites record it (view_syncer.rs wrapper + `sync_query_pipeline_set` transform site) | `init_connection_rejects_a_server_user_id_that_does_not_match`, `desired_queries_transform_rejects_a_server_user_id_that_does_not_match`, `transform_carries_the_api_servers_validation_only_when_uncached`, `validate_returns_the_api_servers_validation` | **CLOSED 2026-09-03** (TS view-syncer.ts:1984-1992, 2749-2767) |
