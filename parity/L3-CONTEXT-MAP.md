...
|---|---|---|---|---|
| per-connection accept handler (`workers/syncer.ts#handleConnection`) | `new Connection` → `connection.init()` (version gate + `connected`), connect-time error/close | **accept task** | `router::handle_connection`, `ws_server::accept_connection`, `ws_server::send_error_and_close` | one per connection, NOT behind any CG lock |
...
| CVR I/O awaited inline (`cvr-store.ts`) | CVR flush / persistence | **offload runtime** (invention I-6) | `sync_engine.rs` offload, CVR flush actor | pool runtime; must preserve durability-before-poke ordering |
...
|---|---|---|---|
| `connected_message` | `router.rs::handle_connection` | accept task | the live emission — decoupled from hydrate (bug-1 fix `5e71e24f4`) |
...
Tier-1 guards the direct enclosing fn in router.rs; Tier-2 extends the same rule
...
a `connected_message` call into `sync_engine.rs::new` fails Tier-2 (a re-coupling
