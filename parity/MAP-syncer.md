...
TS origin files: **16**  ·  Rust files: **26** (5 new)

| TS file (LOC) | rel | Rust file(s) (shared syms) |
|---|---|---|
| `auth/auth.ts` (243) | **MERGED** | `connection_context.rs` (4), `protocol.rs` (1) |
| `auth/jwt.ts` (89) | **MERGED** | `auth.rs` (4) |
| `auth/read-authorizer.ts` (152) | **1:1** | `permissions.rs` (5) |
| `custom-queries/transform-query.ts` (290) | **MERGED** | `custom_query.rs` (5), `pipeline_driver.rs` (1), `auth.rs` (1) |
| `custom/fetch.ts` (569) | **SPLIT** | `metrics.rs` (6), `custom_query.rs` (4), `protocol.rs` (2), `push_relay.rs` (1) |
| `db/lite-tables.ts` (356) | **1:1** | `replica_schema.rs` (5), `pipeline_driver.rs` (1) |
| `services/view-syncer/connection-context-manager.ts` (892) | **MERGED** | `connection_context.rs` (28), `router.rs` (1) |
| `services/view-syncer/drain-coordinator.ts` (76) | **1:1** | `drain.rs` (6) |
| `services/view-syncer/e2e-serving-lag.ts` (82) | **1:1** | `e2e_serving_lag.rs` (6) |
| `services/view-syncer/pipeline-driver.ts` (1558) | **SPLIT** | `pipeline_driver.rs` (10), `advance_gate.rs` (6), `sync_engine.rs` (2), `connection.rs` (1), `connection_context.rs` (1), `metrics.rs` (1), `router.rs` (1) |
| `services/view-syncer/query-covering.ts` (444) | **1:1** | `query_covering.rs` (24), `metrics.rs` (1) |
| `services/view-syncer/view-syncer.ts` (3002) | **SPLIT** | `router.rs` (15), `main.rs` (5), `sync_engine.rs` (2), `protocol.rs` (1) |
| `workers/connect-params.ts` (100) | **1:1** | `connect_params.rs` (2), `ws_server.rs` (1) |
| `workers/connection.ts` (485) | **MERGED** | `connection.rs` (10), `ws_sink.rs` (1) |
| `workers/syncer-ws-message-handler.ts` (283) | **1:1** | `message_handler.rs` (2) |
| `workers/syncer.ts` (759) | **1:1** | `serving_lag.rs` (13), `router.rs` (1), `main.rs` (1), `ws_server.rs` (1) |

**New Rust files (no TS origin — added in the port):**  `http_server.rs` (454), `lib.rs` (77), `live_count.rs` (127), `otel.rs` (131), `trace.rs` (30)

**Merges (many TS → one Rust file):**
- `auth.rs` ⟵ `auth/jwt.ts`, `custom-queries/transform-query.ts`
- `connection.rs` ⟵ `services/view-syncer/pipeline-driver.ts`, `workers/connection.ts`
- `connection_context.rs` ⟵ `auth/auth.ts`, `services/view-syncer/connection-context-manager.ts`, `services/view-syncer/pipeline-driver.ts`
- `custom_query.rs` ⟵ `custom-queries/transform-query.ts`, `custom/fetch.ts`
- `main.rs` ⟵ `services/view-syncer/view-syncer.ts`, `workers/syncer.ts`
- `metrics.rs` ⟵ `custom/fetch.ts`, `services/view-syncer/pipeline-driver.ts`, `services/view-syncer/query-covering.ts`
- `pipeline_driver.rs` ⟵ `custom-queries/transform-query.ts`, `db/lite-tables.ts`, `services/view-syncer/pipeline-driver.ts`
- `protocol.rs` ⟵ `auth/auth.ts`, `custom/fetch.ts`, `services/view-syncer/view-syncer.ts`
- `router.rs` ⟵ `services/view-syncer/connection-context-manager.ts`, `services/view-syncer/pipeline-driver.ts`, `services/view-syncer/view-syncer.ts`, `workers/syncer.ts`
...
### `connect_params.rs`  ⟵  `workers/connect-params.ts`
...
| `Connection` (workers/connection.ts:78) | `Connection` (:58) | exact |
...
| `init` (services/view-syncer/pipeline-driver.ts:325) | `init` (:124) | exact |
| `MessageHandler` (workers/connection.ts:51) | `MessageHandler` (:39) | exact |
| `send` (workers/connection.ts:348) | `send` (:248) | exact |
| `sendError` (workers/connection.ts:356) | `send_error` (:256) | exact |

🟥 **TS symbols not resolved into this file (1):** `StreamResult`

🟦 **Rust-only added here (14):** `DOWNSTREAM_MSG_INTERVAL_MS`, `LogLevel`, `WsState`, `classify_error_log_level`, `client_id`, `close_with_error`, `handle_close`, `handle_error`, `handle_inbound`, `handle_result`, `is_closed`, `maybe_send_pong`, `new`, `ws_id`
...
| `closeConnection` (services/view-syncer/connection-context-manager.ts:136) | `close_connection` (:614) | exact |
...
| `GroupAuthState` (services/view-syncer/connection-context-manager.ts:95) | `GroupAuthState` (:113) | exact |
| `HeaderOptions` (services/view-syncer/connection-context-manager.ts:44) | `HeaderOptions` (:64) | exact |
| `initConnection` (services/view-syncer/connection-context-manager.ts:111) | `init_connection` (:479) | exact |
...
| `mustGetConnectionContext` (services/view-syncer/connection-context-manager.ts:152) | `must_get_connection_context` (:677) | exact |
...
| `updateAuth` (services/view-syncer/connection-context-manager.ts:116) | `update_auth` (:505) | exact |
...
🟦 **Rust-only added here (21):** `CCMError`, `ConnectParamsForRegistration`, `InitConnectionBody`, `JwtPayload`, `MaintenanceKind`, `MaintenancePlan`, `UpdateAuthBody`, `ValidationResult`, `build_fetch_context`, `demote_connection`, `next_revalidate_at`, `now`, `now_ms`, `raw`, `refresh_background_connection_context`, `remove_connection_internal`, `resolve_auth`, `set_background_connection`, `store_connection`, `to_error_body`, `update_background_retransform_deadline`
...
| `getCacheKey` (custom-queries/transform-query.ts:259) | `get_cache_key` (:481) | exact |
| `normalizedHeaders` (custom-queries/transform-query.ts:278) | `normalized_headers` (:474) | exact |
...
### `drain.rs`  ⟵  `services/view-syncer/drain-coordinator.ts`
...
🟦 **Rust-only added here (3):** `FORCE_DRAIN_PADDING`, `TARGET_UTILIZATION`, `default`

### `e2e_serving_lag.rs`  ⟵  `services/view-syncer/e2e-serving-lag.ts`
...
### `live_count.rs`  ⟵  _(new)_


🟦 **Rust-only added here (10):** `CLIENT_GROUP`, `Guard`, `PUSHER`, `SYNC_ENGINE`, `WS_MESSAGE_HANDLER`, `dec`, `drop`, `drop_backtrace`, `inc`, `snapshot`
...
### `message_handler.rs`  ⟵  `workers/syncer-ws-message-handler.ts`
...
🟦 **Rust-only added here (12):** `ConnContextInfo`, `ConnContextManagerDispatch`, `MutagenDispatch`, `PushOverride`, `PushRelayHeaders`, `PusherDispatch`, `ViewSyncerDispatch`, `ack_mutation_responses`, `delete_client_mutations`, `enqueue_push`, `handle_push`, `process_mutation`

### `metrics.rs`  ⟵  `custom/fetch.ts`, `services/view-syncer/pipeline-driver.ts`, `services/view-syncer/query-covering.ts`

| TS symbol | Rust symbol | match |
|---|---|---|
| `add` (services/view-syncer/query-covering.ts:67) | `add` (:829) | exact |
| `apiAttemptDuration` (custom/fetch.ts:568) | `API_DURATION_BOUNDARIES_S` (:353) | fuzzy 0.50 |
| `apiInFlight` (custom/fetch.ts:116) | `record_api_in_flight` (:445) | fuzzy 0.75 |
| `apiRequestMetricAttrs` (custom/fetch.ts:516) | `api_request_metric_attrs` (:390) | exact |
| `recordApiAttempt` (custom/fetch.ts:549) | `record_api_attempt` (:413) | exact |
| `reset` (services/view-syncer/pipeline-driver.ts:343) | `record_reset` (:855) | fuzzy 0.50 |

🟥 **TS symbols not resolved into this file (1):** `FetchMetricsOptions`

🟦 **Rust-only added here (61):** `ApiOtel`, `C`, `CvrAttemptOtel`, `G`, `GAUGES`, `HIST_BOUNDS_SECS`, `Histogram`, `I`, `INSTRUMENT`, `INSTRUMENTS`, `Metrics`, `OTEL_LATENCY_BOUNDARIES_S`, `Otel`, `QueryTransformOtel`, `ServingLagOtel`, `WS_QUEUED_BYTES`, `WS_QUEUED_FRAMES`, `active_clients`, `cvr_flush_failures`, `failed_client_groups`, `fmt`, `observe_millis`, `observe_secs`, `proto_attr`, `record_active_client_delta`, `record_advance`, `record_api_request`, `record_api_request_duration`, `record_cvr_flush_attempt`, `record_cvr_flush_failure`, `record_cvr_load_attempt`, `record_e2e_serving_lag`, `record_e2e_serving_lag_clamp`, `record_fail_group`, `record_hydration`, `record_query_transformation`, `record_query_transformation_hash_change`, `record_query_transformation_no_op`, `record_query_transformation_time`, `record_view_syncer_hydration`, `record_view_syncer_lag_ms`, `record_ws_connection_attempt`, `record_ws_connection_failure`, `record_ws_connection_success`, `record_ws_open_delta`, `record_ws_queued_bytes_delta`, `record_ws_queued_delta`, `record_ws_shed`, `register_cvr_pool_gauges`, `register_serving_lag_gauges`, `render`, `render_prometheus`, `view_syncer_hydration_otel`, `view_syncer_lag_otel`, `ws_connection_attempts`, `ws_connection_failures`, `ws_connection_successes`, `ws_open_connections`, `ws_queued_bytes_gauge`, `ws_queued_frames_gauge`, `ws_sheds`
...
### `permissions.rs`  ⟵  `auth/read-authorizer.ts`
...
🟦 **Rust-only added here (49):** `AckMutationResponsesBody`, `AnalyzeQueryOptions`, `BackoffBody`, `BasicErrorBody`, `ChangeDesiredQueriesBody`, `ConnectedBody`, `ConnectedMessage`, `DecodeError`, `DeleteClientsBody`, `ErrorKind`, `ErrorMessage`, `ErrorOrigin`, `InspectUpBody`, `MIN_SERVER_SUPPORTED_SYNC_PROTOCOL`, `MutationID`, `MutationPatchOp`, `PROTOCOL_VERSION`, `PokeEndBody`, `PokePartBody`, `PokeStartBody`, `PongBody`, `PongMessage`, `PushBody`, `PushFailedServerBody`, `PushFailedZeroCacheBody`, `QueriesClearOp`, `QueriesDelOp`, `QueriesPatchOp`, `QueriesPutOp`, `RowPatchOp`, `SchemaVersions`, `SecProtocols`, `TransformFailedServerBody`, `TransformFailedZeroCacheBody`, `Upstream`, `basic`, `client_not_found`, `decode_sec_protocols`, `downstream_message`, `internal`, `invalid_message`, `invalid_push`, `kind`, `message`, `parse_upstream`, `parse_upstream_array`, `rehome`, `unauthorized`, `version_not_supported`

### `push_relay.rs`  ⟵  `custom/fetch.ts`

| TS symbol | Rust symbol | match |
|---|---|---|
| `getBodyPreview` (custom/fetch.ts:62) | `BODY_PREVIEW_CAP` (:38) | fuzzy 0.67 |

🟦 **Rust-only added here (12):** `CLEANUP_RESULTS_MUTATION_NAME`, `DEFAULT_QUEUE_CAP`, `HttpRelayPusher`, `PushTarget`, `QueuedPush`, `RELAY_TIMEOUT`, `cleanup_push_body`, `enqueue_payload`, `mutation_ids_of`, `queue_cap`, `read_body_preview`, `relay_body`

### `query_covering.rs`  ⟵  `services/view-syncer/query-covering.ts`
...
### `replica_schema.rs`  ⟵  `db/lite-tables.ts`
...
| TS symbol | Rust symbol | match |
|---|---|---|
| `checkClientAndCVRVersions` (services/view-syncer/view-syncer.ts:2875) | `check_client_and_cvr_versions` (:54) | exact |
| `drain` (workers/syncer.ts:732) | `drain` (:1017) | exact |
| `queryCount` (services/view-syncer/view-syncer.ts:658) | `query_count` (:1609) | exact |
| `rowCount` (services/view-syncer/view-syncer.ts:662) | `row_count` (:1615) | exact |
| `servedVersion` (services/view-syncer/view-syncer.ts:666) | `mark_version_served` (:2905) | fuzzy 0.67 |
| `servingLagEligible` (services/view-syncer/view-syncer.ts:670) | `serving_lag_eligible` (:1603) | exact |
| `TTL_CLOCK_INTERVAL` (services/view-syncer/view-syncer.ts:202) | `get_ttl_clock` (:1639) | fuzzy 0.67 |
| `TTL_TIMER_HYSTERESIS` (services/view-syncer/view-syncer.ts:210) | `TTL_TIMER_HYSTERESIS_MS` (:44) | fuzzy 0.75 |
...
### `serving_lag.rs`  ⟵  `workers/syncer.ts`

| TS symbol | Rust symbol | match |
|---|---|---|
| `boundReplicaReadyStates` (workers/syncer.ts:82) | `bound_replica_ready_states` (:82) | exact |
| `computeMaxServingLagMs` (workers/syncer.ts:247) | `compute_max_serving_lag_ms` (:234) | exact |
| `computeServingLagDistributionMs` (workers/syncer.ts:174) | `compute_serving_lag_distribution_ms` (:177) | exact |
| `computeServingLagStatsMs` (workers/syncer.ts:226) | `compute_serving_lag_stats_ms` (:225) | exact |
| `findFirstUnservedIndex` (workers/syncer.ts:138) | `find_first_unserved_index` (:143) | exact |
| `lowerBoundReplicaReadyTimeMs` (workers/syncer.ts:104) | `lower_bound_replica_ready_time_ms` (:104) | exact |
| `MAX_REPLICA_READY_STATES` (workers/syncer.ts:76) | `MAX_REPLICA_READY_STATES` (:18) | exact |
| `percentileNearestRank` (workers/syncer.ts:160) | `percentile_nearest_rank` (:162) | exact |
| `pruneReplicaReadyStates` (workers/syncer.ts:93) | `prune_replica_ready_states` (:91) | exact |
| `ReplicaReadyState` (workers/syncer.ts:52) | `ReplicaReadyState` (:25) | exact |
| `ServingLagStats` (workers/syncer.ts:62) | `ServingLagStats` (:42) | exact |
| `ServingLagViewSyncer` (workers/syncer.ts:57) | `ServingLagViewSyncer` (:34) | exact |
| `upperBoundWatermark` (workers/syncer.ts:121) | `upper_bound_watermark` (:123) | exact |
...
| TS symbol | Rust symbol | match |
|---|---|---|
| `accumulate` (services/view-syncer/pipeline-driver.ts:1276) | `accumulate_signature` (:1619) | fuzzy 0.50 |
| `hasExpiredQueries` (services/view-syncer/view-syncer.ts:2933) | `remove_expired_queries` (:1399) | fuzzy 0.50 |
| `RowChange` (services/view-syncer/pipeline-driver.ts:83) | `row_change_to_maps` (:1584) | fuzzy 0.67 |
...
| `Auth` | auth/auth.ts:25 | `Auth` connection_context.rs:82 | ✅ exact |
| `ValidateLegacyJWT` | auth/auth.ts:27 | — | 🟥 UNRESOLVED |
| `isProvidedAuth` | auth/auth.ts:32 | connection_context.rs is_some_and non-empty | 📌 inlined |
| `authEquals` | auth/auth.ts:36 | `auth_equals` connection_context.rs:339 | ✅ exact |
| `pickToken` | auth/auth.ts:126 | `pick_token` connection_context.rs:273 | ✅ exact |
| `isAuthErrorBody` | auth/auth.ts:211 | `ErrorBody` protocol.rs:192 | 🔁 rename 0.67 |
| `getRemoteKeyset` | auth/jwt.ts:32 | auth.rs JWKS_CACHE/lookup_cached_jwk | 📌 cached remote JWKS |
| `tokenConfigOptions` | auth/jwt.ts:41 | — | 🟥 UNRESOLVED |
| `loadJwk` | auth/jwt.ts:73 | auth.rs serde_json::from_str | 📌 parse JWK |
| `loadSecret` | auth/jwt.ts:77 | auth.rs DecodingKey::from_secret | 📌 secret key |
| `verifyTokenImpl` | auth/jwt.ts:81 | auth.rs verify_sync/verify_with_jwk(s) | 📌 JWT verify (split sync/async) |
| `TransformedAndHashed` | auth/read-authorizer.ts:10 | — | 🟥 UNRESOLVED |
| `transformAndHashQuery` | auth/read-authorizer.ts:24 | `transform_and_hash_query` permissions.rs:63 | ✅ exact |
| `transformQuery` | auth/read-authorizer.ts:45 | `transform_query` permissions.rs:79 | ✅ exact |
| `transformQueryInternal` | auth/read-authorizer.ts:61 | `transform_query_internal` permissions.rs:86 | ✅ exact |
| `addRulesToWhere` | auth/read-authorizer.ts:105 | `add_rules_to_where` permissions.rs:135 | ✅ exact |
| `transformCondition` | auth/read-authorizer.ts:127 | `transform_condition` permissions.rs:146 | ✅ exact |
...
| `CustomQueryTransformer` | custom-queries/transform-query.ts:82 | `CustomQueryContext` custom_query.rs:48 | 🔁 rename 0.50 |
| `destroy` | custom-queries/transform-query.ts:98 | `destroy` pipeline_driver.rs:553 | ✅ exact |
| `validate` | custom-queries/transform-query.ts:111 | `validate_auth` auth.rs:381 | 🔁 rename 0.50 |
| `transform` | custom-queries/transform-query.ts:117 | `post_transform` custom_query.rs:278 | 🔁 rename 0.50 |
| `getCacheKey` | custom-queries/transform-query.ts:259 | `get_cache_key` custom_query.rs:481 | ✅ exact |
| `normalizedHeaders` | custom-queries/transform-query.ts:278 | `normalized_headers` custom_query.rs:474 | ✅ exact |
| `compileUrlPattern` | custom/fetch.ts:52 | N/A | 📌 no separate compile step; url_match matches the raw pattern inline |
| `getBodyPreview` | custom/fetch.ts:62 | `BODY_PREVIEW_CAP` push_relay.rs:38 | 🔁 rename 0.67 |
| `FetchMetricsOptions` | custom/fetch.ts:92 | — | 🟥 UNRESOLVED |
| `apiInFlight` | custom/fetch.ts:116 | `record_api_in_flight` metrics.rs:445 | 🔁 rename 0.75 |
| `urlMatch` | custom/fetch.ts:389 | `url_match` custom_query.rs:121 | ✅ exact |
| `getBackoffDelayMs` | custom/fetch.ts:407 | `get_backoff_delay_ms` custom_query.rs:261 | ✅ exact |
| `apiFailedBody` | custom/fetch.ts:411 | `PushFailedHttpBody` protocol.rs:127 | 🔁 rename 0.40 |
| `apiErrorFromResult` | custom/fetch.ts:462 | custom_query.rs response validation | 📌 error extraction |
| `legacyPushErrorReason` | custom/fetch.ts:484 | `ErrorReason` protocol.rs:64 | 🔁 rename 0.50 |
| `apiRequestMetricAttrs` | custom/fetch.ts:516 | `api_request_metric_attrs` metrics.rs:390 | ✅ exact |
| `apiResponseErrorMetricAttrs` | custom/fetch.ts:528 | metrics.rs record_api_attempt attrs | 📌 status attrs |
| `recordApiAttempt` | custom/fetch.ts:549 | `record_api_attempt` metrics.rs:413 | ✅ exact |
| `apiAttempts` | custom/fetch.ts:567 | metrics.rs record_api_attempt | 📌 OTel counter |
| `apiAttemptDuration` | custom/fetch.ts:568 | `API_DURATION_BOUNDARIES_S` metrics.rs:353 | 🔁 rename 0.50 |
| `LiteTableSpecWithReplicationStatus` | db/lite-tables.ts:37 | — | 🟥 UNRESOLVED |
| `listTables` | db/lite-tables.ts:47 | `list_tables` replica_schema.rs:273 | ✅ exact |
| `listIndexes` | db/lite-tables.ts:141 | `list_unique_indexes` replica_schema.rs:200 | 🔁 rename 0.67 |
| `ZqlSpecOptions` | db/lite-tables.ts:184 | — | 🟥 UNRESOLVED |
| `computeZqlSpecs` | db/lite-tables.ts:210 | `compute_zql_specs` replica_schema.rs:79 | ✅ exact |
| `computeZqlSpecsFromLiteSpecs` | db/lite-tables.ts:227 | `compute_table_specs_from_path` replica_schema.rs:73 | 🔁 rename 0.43 |
| `mustGetTableSpec` | db/lite-tables.ts:326 | `IvmTableSpec` pipeline_driver.rs:49 | 🔁 rename 0.50 |
| `keyCmp` | db/lite-tables.ts:343 | replica_schema.rs sort_by len-then-lex | 📌 inlined key compare |
| `ConnectionState` | services/view-syncer/connection-context-manager.ts:17 | `ConnectionState` connection_context.rs:29 | ✅ exact |
| `UserState` | services/view-syncer/connection-context-manager.ts:23 | `UserState` connection_context.rs:36 | ✅ exact |
| `ConnectionValidation` | services/view-syncer/connection-context-manager.ts:30 | `ConnectionValidation` connection_context.rs:42 | ✅ exact |
| `ConnectionSelector` | services/view-syncer/connection-context-manager.ts:37 | `ConnectionSelector` connection_context.rs:49 | ✅ exact |
| `HeaderOptions` | services/view-syncer/connection-context-manager.ts:44 | `HeaderOptions` connection_context.rs:64 | ✅ exact |
| `ConnectionFetchContext` | services/view-syncer/connection-context-manager.ts:54 | `ConnectionFetchContext` connection_context.rs:73 | ✅ exact |
| `ConnectionContext` | services/view-syncer/connection-context-manager.ts:65 | `ConnectionContext` connection_context.rs:95 | ✅ exact |
| `GroupAuthState` | services/view-syncer/connection-context-manager.ts:95 | `GroupAuthState` connection_context.rs:113 | ✅ exact |
| `ConnectionContextManager` | services/view-syncer/connection-context-manager.ts:104 | `ConnectionContextManager` connection_context.rs:368 | ✅ exact |
| `registerConnection` | services/view-syncer/connection-context-manager.ts:105 | `register_connection` connection_context.rs:410 | ✅ exact |
| `initConnection` | services/view-syncer/connection-context-manager.ts:111 | `init_connection` connection_context.rs:479 | ✅ exact |
| `updateAuth` | services/view-syncer/connection-context-manager.ts:116 | `update_auth` connection_context.rs:505 | ✅ exact |
| `validateConnection` | services/view-syncer/connection-context-manager.ts:121 | `validate_connection` connection_context.rs:540 | ✅ exact |
| `failConnection` | services/view-syncer/connection-context-manager.ts:132 | `fail_connection` connection_context.rs:604 | ✅ exact |
| `closeConnection` | services/view-syncer/connection-context-manager.ts:136 | `close_connection` connection_context.rs:614 | ✅ exact |
| `markBackgroundRetransformSuccess` | services/view-syncer/connection-context-manager.ts:140 | `mark_background_retransform_success` connection_context.rs:620 | ✅ exact |
| `setSharedRetransformReady` | services/view-syncer/connection-context-manager.ts:145 | `set_shared_retransform_ready` connection_context.rs:640 | ✅ exact |
| `deferMaintenance` | services/view-syncer/connection-context-manager.ts:147 | `defer_maintenance` connection_context.rs:650 | ✅ exact |
| `getConnectionContext` | services/view-syncer/connection-context-manager.ts:149 | `get_connection_context` connection_context.rs:666 | ✅ exact |
| `mustGetConnectionContext` | services/view-syncer/connection-context-manager.ts:152 | `must_get_connection_context` connection_context.rs:677 | ✅ exact |
| `getBackgroundConnectionContext` | services/view-syncer/connection-context-manager.ts:156 | `get_background_connection_context` connection_context.rs:688 | ✅ exact |
| `mustGetBackgroundConnectionContext` | services/view-syncer/connection-context-manager.ts:157 | `must_get_background_connection_context` connection_context.rs:693 | ✅ exact |
| `getGroupState` | services/view-syncer/connection-context-manager.ts:159 | `get_group_state` connection_context.rs:701 | ✅ exact |
| `planMaintenance` | services/view-syncer/connection-context-manager.ts:161 | `plan_maintenance` connection_context.rs:707 | ✅ exact |
| `ConnectionContextManagerImpl` | services/view-syncer/connection-context-manager.ts:176 | — | 🟥 UNRESOLVED |
| `compareByInsertionOrder` | services/view-syncer/connection-context-manager.ts:844 | `compare_by_insertion_order` connection_context.rs:900 | ✅ exact |
| `comparePreferredValidatedConnection` | services/view-syncer/connection-context-manager.ts:851 | `compare_preferred_validated_connection` connection_context.rs:907 | ✅ exact |
| `minDefined` | services/view-syncer/connection-context-manager.ts:858 | `min_defined` connection_context.rs:891 | ✅ exact |
| `sameConnectionSelector` | services/view-syncer/connection-context-manager.ts:868 | connection_context.rs set_background_connection | 📌 inlined tuple match |
| `filterHeaders` | services/view-syncer/connection-context-manager.ts:875 | router.rs filtered_query_headers | 📌 header allowlist |
| `DrainCoordinator` | services/view-syncer/drain-coordinator.ts:31 | `DrainCoordinator` drain.rs:39 | ✅ exact |
| `draining` | services/view-syncer/drain-coordinator.ts:37 | `is_draining` drain.rs:105 | 🔁 rename 1.00 |
| `shouldDrain` | services/view-syncer/drain-coordinator.ts:41 | `should_drain` drain.rs:57 | ✅ exact |
| `drainNextIn` | services/view-syncer/drain-coordinator.ts:45 | `drain_next_in` drain.rs:66 | ✅ exact |
| `forceDrainTimeout` | services/view-syncer/drain-coordinator.ts:66 | `force_drain_timeout` drain.rs:85 | ✅ exact |
| `nextDrainTime` | services/view-syncer/drain-coordinator.ts:71 | `next_drain_time` drain.rs:110 | ✅ exact |
| `PendingUpstreamCommit` | services/view-syncer/e2e-serving-lag.ts:3 | `PendingUpstreamCommit` e2e_serving_lag.rs:14 | ✅ exact |
| `E2EServingLagTracker` | services/view-syncer/e2e-serving-lag.ts:19 | `E2EServingLagTracker` e2e_serving_lag.rs:29 | ✅ exact |
| `pending` | services/view-syncer/e2e-serving-lag.ts:22 | `pending` e2e_serving_lag.rs:38 | ✅ exact |
| `onVersionReady` | services/view-syncer/e2e-serving-lag.ts:35 | `on_version_ready` e2e_serving_lag.rs:50 | ✅ exact |
| `onVersionServed` | services/view-syncer/e2e-serving-lag.ts:55 | `on_version_served` e2e_serving_lag.rs:72 | ✅ exact |
| `Observation` | services/view-syncer/e2e-serving-lag.ts:77 | `Observation` e2e_serving_lag.rs:21 | ✅ exact |
...
| `RowChange` | services/view-syncer/pipeline-driver.ts:83 | `row_change_to_maps` sync_engine.rs:1584 | 🔁 rename 0.67 |
...
| `init` | services/view-syncer/pipeline-driver.ts:325 | `init` connection.rs:124 | ✅ exact |
| `initialized` | services/view-syncer/pipeline-driver.ts:334 | `initialized` pipeline_driver.rs:209 | ✅ exact |
| `reset` | services/view-syncer/pipeline-driver.ts:343 | `record_reset` metrics.rs:855 | 🔁 rename 0.50 |
| `replicaVersion` | services/view-syncer/pipeline-driver.ts:386 | pipeline_driver.rs snapshotter current_version | 📌 field/getter |
| `currentVersion` | services/view-syncer/pipeline-driver.ts:395 | `current_version` pipeline_driver.rs:214 | ✅ exact |
...
| `removeQuery` | services/view-syncer/pipeline-driver.ts:834 | `remove_query` pipeline_driver.rs:381 | ✅ exact |
| `rowSetSignature` | services/view-syncer/pipeline-driver.ts:874 | `row_set_signature` pipeline_driver.rs:541 | ✅ exact |
| `getRow` | services/view-syncer/pipeline-driver.ts:906 | `get_row` pipeline_driver.rs:535 | ✅ exact |
| `advance` | services/view-syncer/pipeline-driver.ts:923 | `advance` pipeline_driver.rs:457 | ✅ exact |
| `accumulate` | services/view-syncer/pipeline-driver.ts:1276 | `accumulate_signature` sync_engine.rs:1619 | 🔁 rename 0.50 |
...
| `fetch` | services/view-syncer/pipeline-driver.ts:1428 | `FetchConfig` connection_context.rs:135 | 🔁 rename 0.50 |
...
| `buildPrimaryKeys` | services/view-syncer/pipeline-driver.ts:1520 | `set_client_primary_keys` pipeline_driver.rs:373 | 🔁 rename 0.50 |
...
| `RunningQuery` | services/view-syncer/query-covering.ts:15 | `RunningQuery` query_covering.rs:24 | ✅ exact |
| `CoveringQuery` | services/view-syncer/query-covering.ts:21 | `CoveringQuery` query_covering.rs:33 | ✅ exact |
| `isQueryCoveredBy` | services/view-syncer/query-covering.ts:40 | `is_query_covered_by` query_covering.rs:97 | ✅ exact |
| `findCoveringQuery` | services/view-syncer/query-covering.ts:44 | `find_covering_query` query_covering.rs:106 | ✅ exact |
| `QueryCoveringIndex` | services/view-syncer/query-covering.ts:55 | `QueryCoveringIndex` query_covering.rs:120 | ✅ exact |
| `add` | services/view-syncer/query-covering.ts:67 | `add` metrics.rs:829 | ✅ exact |
| `remove` | services/view-syncer/query-covering.ts:81 | `remove` query_covering.rs:158 | ✅ exact |
| `rootKey` | services/view-syncer/query-covering.ts:125 | `root_key` query_covering.rs:193 | ✅ exact |
| `astCoveredBy` | services/view-syncer/query-covering.ts:129 | `ast_covered_by` query_covering.rs:202 | ✅ exact |
| `boundsCoveredBy` | services/view-syncer/query-covering.ts:143 | `bounds_covered_by` query_covering.rs:217 | ✅ exact |
| `relatedCoveredBy` | services/view-syncer/query-covering.ts:170 | `related_covered_by` query_covering.rs:242 | ✅ exact |
| `conditionEquivalent` | services/view-syncer/query-covering.ts:192 | `condition_equivalent` query_covering.rs:259 | ✅ exact |
| `conditionImplies` | services/view-syncer/query-covering.ts:199 | `condition_implies` query_covering.rs:263 | ✅ exact |
| `correlatedConditionImplies` | services/view-syncer/query-covering.ts:237 | `correlated_condition_implies` query_covering.rs:303 | ✅ exact |
| `sameRelatedEdge` | services/view-syncer/query-covering.ts:262 | `same_related_edge` query_covering.rs:326 | ✅ exact |
| `simpleConditionImplies` | services/view-syncer/query-covering.ts:274 | `simple_condition_implies` query_covering.rs:333 | ✅ exact |
| `equalityImplies` | services/view-syncer/query-covering.ts:315 | `equality_implies` query_covering.rs:371 | ✅ exact |
| `orderConditionImplies` | services/view-syncer/query-covering.ts:365 | `order_condition_implies` query_covering.rs:387 | ✅ exact |
| `columnLiteralParts` | services/view-syncer/query-covering.ts:399 | `ColumnLiteralParts` query_covering.rs:409 | ✅ exact |
| `isEqualityOp` | services/view-syncer/query-covering.ts:416 | `is_equality_op` query_covering.rs:430 | ✅ exact |
| `isNumericOrderOp` | services/view-syncer/query-covering.ts:420 | `is_numeric_order_op` query_covering.rs:434 | ✅ exact |
| `isNonNullScalarLiteralValue` | services/view-syncer/query-covering.ts:426 | `is_non_null_scalar_literal_value` query_covering.rs:438 | ✅ exact |
| `literalArrayIncludes` | services/view-syncer/query-covering.ts:432 | `literal_array_includes` query_covering.rs:442 | ✅ exact |
| `jsonEqual` | services/view-syncer/query-covering.ts:439 | `json_equal` query_covering.rs:462 | ✅ exact |
...
| `TTL_CLOCK_INTERVAL` | services/view-syncer/view-syncer.ts:202 | `get_ttl_clock` router.rs:1639 | 🔁 rename 0.67 |
| `TTL_TIMER_HYSTERESIS` | services/view-syncer/view-syncer.ts:210 | `TTL_TIMER_HYSTERESIS_MS` router.rs:44 | 🔁 rename 0.75 |
...
| `queryCount` | services/view-syncer/view-syncer.ts:658 | `query_count` router.rs:1609 | ✅ exact |
| `rowCount` | services/view-syncer/view-syncer.ts:662 | `row_count` router.rs:1615 | ✅ exact |
| `servedVersion` | services/view-syncer/view-syncer.ts:666 | `mark_version_served` router.rs:2905 | 🔁 rename 0.67 |
| `servingLagEligible` | services/view-syncer/view-syncer.ts:670 | `serving_lag_eligible` router.rs:1603 | ✅ exact |
...
| `checkClientAndCVRVersions` | services/view-syncer/view-syncer.ts:2875 | `check_client_and_cvr_versions` router.rs:54 | ✅ exact |
...
| `hasExpiredQueries` | services/view-syncer/view-syncer.ts:2933 | `remove_expired_queries` sync_engine.rs:1399 | 🔁 rename 0.50 |
...
| `ConnectParams` | workers/connect-params.ts:9 | `ConnectParams` connect_params.rs:10 | ✅ exact |
| `normalizeHeaders` | workers/connect-params.ts:32 | ws_server.rs (dup-header join) | 📌 header normalization |
| `getConnectParams` | workers/connect-params.ts:45 | `get_connect_params` connect_params.rs:50 | ✅ exact |
| `HandlerResult` | workers/connection.ts:31 | `HandlerResult` connection.rs:26 | ✅ exact |
| `StreamResult` | workers/connection.ts:45 | — | 🟥 UNRESOLVED |
| `MessageHandler` | workers/connection.ts:51 | `MessageHandler` connection.rs:39 | ✅ exact |
| `handleMessage` | workers/connection.ts:52 | `handle_message` connection.rs:42 | ✅ exact |
| `Connection` | workers/connection.ts:78 | `Connection` connection.rs:58 | ✅ exact |
| `close` | workers/connection.ts:168 | `close` connection.rs:231 | ✅ exact |
| `handleInitConnection` | workers/connection.ts:190 | `handle_init_connection` connection.rs:306 | ✅ exact |
| `send` | workers/connection.ts:348 | `send` connection.rs:248 | ✅ exact |
| `sendError` | workers/connection.ts:356 | `send_error` connection.rs:256 | ✅ exact |
| `WebSocketLike` | workers/connection.ts:361 | `DirectWebSocketSink` ws_sink.rs:77 | 🔁 rename 0.40 |
| `findProtocolError` | workers/connection.ts:433 | connection.rs classify_error_log_level | 📌 protocol-error classify |
...
| `isTransientSocketMessage` | workers/connection.ts:477 | connection.rs (message substring) | 📌 transient downgrade |
| `withTraceparent` | workers/syncer-ws-message-handler.ts:28 | `with_traceparent` message_handler.rs:25 | ✅ exact |
| `SyncerWsMessageHandler` | workers/syncer-ws-message-handler.ts:36 | `SyncerWsMessageHandler` message_handler.rs:155 | ✅ exact |
| `SyncerWorkerData` | workers/syncer.ts:48 | — | 🟥 UNRESOLVED |
| `ReplicaReadyState` | workers/syncer.ts:52 | `ReplicaReadyState` serving_lag.rs:25 | ✅ exact |
| `ServingLagViewSyncer` | workers/syncer.ts:57 | `ServingLagViewSyncer` serving_lag.rs:34 | ✅ exact |
| `ServingLagStats` | workers/syncer.ts:62 | `ServingLagStats` serving_lag.rs:42 | ✅ exact |
| `MAX_REPLICA_READY_STATES` | workers/syncer.ts:76 | `MAX_REPLICA_READY_STATES` serving_lag.rs:18 | ✅ exact |
| `boundReplicaReadyStates` | workers/syncer.ts:82 | `bound_replica_ready_states` serving_lag.rs:82 | ✅ exact |
| `pruneReplicaReadyStates` | workers/syncer.ts:93 | `prune_replica_ready_states` serving_lag.rs:91 | ✅ exact |
| `lowerBoundReplicaReadyTimeMs` | workers/syncer.ts:104 | `lower_bound_replica_ready_time_ms` serving_lag.rs:104 | ✅ exact |
| `upperBoundWatermark` | workers/syncer.ts:121 | `upper_bound_watermark` serving_lag.rs:123 | ✅ exact |
| `findFirstUnservedIndex` | workers/syncer.ts:138 | `find_first_unserved_index` serving_lag.rs:143 | ✅ exact |
| `percentileNearestRank` | workers/syncer.ts:160 | `percentile_nearest_rank` serving_lag.rs:162 | ✅ exact |
| `computeServingLagDistributionMs` | workers/syncer.ts:174 | `compute_serving_lag_distribution_ms` serving_lag.rs:177 | ✅ exact |
| `computeServingLagStatsMs` | workers/syncer.ts:226 | `compute_serving_lag_stats_ms` serving_lag.rs:225 | ✅ exact |
| `computeMaxServingLagMs` | workers/syncer.ts:247 | `compute_max_serving_lag_ms` serving_lag.rs:234 | ✅ exact |
...
| `drain` | workers/syncer.ts:732 | `drain` router.rs:1017 | ✅ exact |
