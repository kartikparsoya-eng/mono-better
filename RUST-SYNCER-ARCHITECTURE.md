...
Every Rust module names its TS origin in its doc-comment. The full map below is the *file→file* mapping; for *behavior-level* parity (exact matches, intentional divergences, release-gated differences) see [`RUST-SYNCER-TS-PARITY.md`](./RUST-SYNCER-TS-PARITY.md).

### rust-syncer

| Rust file | TS origin (`packages/zero-cache/src/…`) |
|---|---|
| `ws_server.rs` | `workers/syncer.ts` + `workers/connection.ts` (WS accept + lifecycle) |
| `connection.rs` | `workers/connection.ts` (Connection class) |
| `connect_params.rs` | `workers/connect-params.ts` |
| `connection_context.rs` | `services/…/connection-context-manager.ts` |
| `message_handler.rs` | `workers/syncer-ws-message-handler.ts` |
| `router.rs` | connection lifecycle in `syncer.ts` (+ doc-91 sharded executors) |
| `sync_engine.rs` | view-syncer engine + CVR hot path (Phase 7 Stage B) |
| `pipeline_driver.rs` | the IVM bridge (Phase 7 Stage A) |
| `auth.rs` | `auth/jwt.ts` + `auth.ts` |
| `permissions.rs` | read-permission query transformation |
| `custom_query.rs` | `custom-queries/transform-query.ts` (`CustomQueryTransformer`) |
| `query_covering.rs` | `view-syncer/query-covering.ts` (#6182) |
| `e2e_serving_lag.rs` | `view-syncer/e2e-serving-lag.ts` (#6157/#6312) |
| `drain.rs` | `drain-coordinator.ts` |
| `push_relay.rs` | custom-mutation push relay (Option-A write path) |
| `protocol.rs` | `packages/zero-protocol/src/*` valita schemas |
| `metrics.rs` / `otel.rs` | view-syncer OTel instruments + `server/otel-start.ts` |
| `http_server.rs` | `/statz`, `/metrics`, `/heapz`, `/notify` endpoints |
| `ws_sink.rs` | replaces `NapiWebSocketSink` + TSFN |
| `replica_schema.rs` | replica introspection (ViewSyncer data path part 1) |

### rust-ivm (← `packages/zql/src/…`)

| Rust | TS |
|---|---|
| `engine/` | the ZQL IVM engine/pipeline |
| `ivm/` (operators) | `ivm/*.ts` operators (filter, join, take, exists, fan-in/out, view) |
| `builder/` | `builder/*.ts` (AST → pipeline) |
| `planner/` | query planner / cost model |
| `sqlite/` | `better-sqlite3` table source |
| `snapshotter/` · `streamer/` | snapshot diffing + row-change streaming |

### rust-cvr (← `packages/zero-cache/src/…`)

| Rust | TS |
|---|---|
| `cvr.rs` · `store.rs` · `updater.rs` | `services/view-syncer/cvr.ts` + `cvr-store.ts` |
| `row_record_cache.rs` | row-record persistence/caching |
| `row_key.rs` | `types/row-key.ts` |
| `row_set_signature.rs` | `services/view-syncer/row-set-signature.ts` |
| `hash.rs` | `packages/shared/src/hash.ts` |
| `version.rs` | CVR version / lexi cookie |
